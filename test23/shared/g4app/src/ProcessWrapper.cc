...
#if (USE_G4REF>0 && USE_G4REF<4100007) || ( USE_G4PUBLIC>0 && USE_G4PUBLIC<41001 )
//
// this provision is for public releases in the 4.10.00-series, e.g. p02 & p03
// and reference releases up to 4.10.00-ref06
//
   hint = ChooseHadronicInteraction( (track.GetDynamicParticle())->GetKineticEnergy(), mat, elm );   
//   hint = ChooseHadronicInteraction( proj, *tNucleus, mat, elm );

#else
// this will work from ref06 (or refo7 ?) and beyond but not with the 4.10.00 series
//
//   hint = ChooseHadronicInteraction( (track.GetDynamicParticle())->GetKineticEnergy(), mat, elm );   
   hint = ChooseHadronicInteraction( proj, *tNucleus, mat, elm );
#endif
