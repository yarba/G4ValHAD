...
#include <boost/algorithm/string.hpp> // for to_lower function
...
void ModelConfigMapper::PrintDefaults( const std::string& model ) // const
{
   ...
   // mod = ToLower(mod);
   // mod = boost::algorithm::to_lower_copy( mod );
   // I think one can do as below
   std::string mod = boost::algorithm::to_lower_copy( model );
...
void ModelConfigMapper::ChangeParameter( const std::string& model, const std::string& param, const double& value, bool verb )
{
   ...
   // NOTE (JVY, 6/11/2026): this is a leftover fromn the G4VMP code
   //                        but it's a useful info so keep it here
   //
...
void ModelConfigMapper::ChangeParameterViaG4UI( const std::string& model, const std::string& param, const double& value, bool verb )
{
   ...
   // par = ToLower(par);
   std::string par = boost::algorithm::to_lower_copy( param );
...
void ModelConfigMapper::ChangeParameterFTFP( const std::string& pname, const double& value, bool verb )
{
   ...
   std::string par = boost::algorithm::to_lower_copy( pname );
   std::map<std::string,std::string>::iterator itr1=(itr->second).find(par /*pname*/ );
   if ( itr1==(itr->second).end() )
   {
