...
const int NModelsMuons=1;
//std::string ModelsMuons[2] = { "stopping", "captureUpdate" };
std::string ModelsMuons[2] = { "captureUpdate", "stopping" };

// int         ColorModel[4]    = { 1, 6, 2, 3 };
int         ColorModel[4]    = { 3, 1, 2, 6 };
...
int findTargetSinger( std::string );
int findTargetSingerExp( std::string );
int findTargetSingerTheo( std::string );
int findTargetSundelin( std::string );
void drawMuMinusNeutMult( std::string );
void drawMuMinusNNeutEKin( std::string );
void drawMuMinusNeutMultRegre( std::string, std::string );
void drawMuMinusNNeutEKinRegre( std::string, std::string ); 
void drawMuMinNMultForTalk( std::string );
void drawMuMinNRateEKinForTalk( std::string ); 
...
double calcChi2NeutMultSinger( std::string target, std::string model, int& NDF, std::string version="." )
{
   ...
   std::cout << " chi2/NDF = " << chi2 << "/" << NDF << " = " << (chi2/NDF) 
             << " for model " << model << " version " << version << std::endl;
...
double calcChi2NNeutEKinSundelin( std::string target, std::string model, int& NDF, std::string version="." )
{
   ...
   std::cout << " chi2/NDF = " << chi2 << "/" << NDF << " = " << (chi2/NDF) 
             << " for model " << model << std::endl;
   
   return chi2;

}

TLatex* formChi2Printout( const double chi2, const int NDF, const int counter, const std::string label, 
                          double xpos=0.1, double ypos=0.07 )
{
   ...
   txt += ( " for " + label );
   TLatex* ltxt = new TLatex( xpos, ypos-size*counter, txt.c_str() );
   ltxt->SetTextSize(size);
   return ltxt;
...
//      gPad->SetLogy(); // ???
      ...
      std::string general = "MC vs Data from P.Singer";
      ...
      {
	 NDF=0;
	 chi2 = calcChi2NeutMultSinger( target, ModelsMuons[m], NDF );
         myc->cd();
	 TLatex* ltxt1 = formChi2Printout( chi2, NDF, m, ModelsMuons[m] );
         ltxt1->Draw();
      }
      ...
      general = "MC vs Data from R.M.Sundelin et al.";
      TLatex* ltxt2 = new TLatex(0.6, 0.10, general.c_str() );
	 ...
	 chi2 = calcChi2NNeutEKinSundelin( target, ModelsMuons[m], NDF );
         myc->cd();
	 TLatex* ltxt1 = formChi2Printout( chi2, NDF, m, ModelsMuons[m], 0.6 );
         ltxt1->Draw();
      }   
      myc->cd();
   ...
   else if ( target == "S" ) // only Sundelin dataset
   {
      ...
      gPad->SetLogy();
      drawMuMinusNNeutEKin(target);
      std::string general = "MC vs Data from R.M.Sundelin et al.";
      TLatex* ltxt = new TLatex(0.1, 0.10, general.c_str() );
      ltxt->SetTextSize(0.03);
      myc->cd();
      ltxt->Draw();
...
void PlotMuMinusRegre( std::string target, std::string model )
{

   std::string output = "mumin-" + target + "-regre.gif";
   int NDF = 0;
   double chi2 = 0.;
...
//      gPad->SetLogy(); // ???
      ...
      std::string general = "MC/" + model + " vs Data from P.Singer";
      TLatex* ltxt = new TLatex(0.1, 0.10, general.c_str() );
      ltxt->SetTextSize(0.03);
      myc->cd();
      ltxt->Draw();
	 ...
	 ltxt1->Draw();
      ...
      general = "MC/" + model + " vs Data from R.M.Sundelin";
      TLatex* ltxt2 = new TLatex(0.6, 0.10, general.c_str() );
      ltxt2->SetTextSize(0.03);
      myc->cd();
      ltxt2->Draw();
   ...
   else if ( target == "S" ) // only Sundelin dataset
   {
      ...
      gPad->SetLogy();
      drawMuMinusNNeutEKinRegre(target,model);
      std::string general = "MC/" + model + " vs Data from R.M.Sundelin";
      TLatex* ltxt = new TLatex( 0.1, 0.10, general.c_str() );
      ltxt->SetTextSize(0.03);
      myc->cd();
      ltxt->Draw();
...
//      histofile += ".root";
   ...
   TLegend* leg = new TLegend(0.55, 0.70, 0.95, 0.9);
