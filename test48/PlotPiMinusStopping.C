...
void drawPiMinus( std::string );
void drawPiMinusRegression( std::string, std::string );
...
//   TLegend* leg = new TLegend(0.55, 0.70, 0.9, 0.9);
   ...
   //   TGraphErrors*  gr1 = new TGraphErrors(NPointsMadey,KENeut,YValue[TargetID],0,YError[TargetID]);
...
void drawPiMinusMC2Data( std::string target )
{
   ...
   for ( int ip=0; ip<NPointsMadey; ip++)
   {
      ...
      YY[ip] = 1.0;
      DYY[ip] = YError[TargetID][ip] / YValue[TargetID][ip] ;
      if ( (YY[ip]+DYY[ip]) > ymax ) ymax = YY[ip]+DYY[ip];
      if ( (YY[ip]-DYY[ip]) < ymin ) ymin = YY[ip]-DYY[ip];
   }

   // TGraph*  gr1 = new TGraphErrors(NPointsMadey,KENeut,YValue[TargetID],0,YError[TargetID]);
   ...
   for ( int m=0; m<NModelsMesons; m++ )
   {
	...
	for (int kk=0; kk<NPointsMadey; kk++ )
	{
	   if ( xx1 < KENeut[kk] && xx1+xx2 > KENeut[kk] )
	   {
	      ...
	      DX[np] = 0.;
	      MC2DataY[np] = yy / YValue[TargetID][kk];
