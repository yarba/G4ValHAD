...
config=test19.harp.piplus.${target}.${momentum}
...
$ROOTSYS/bin/root -b -p -q HARPModels.C\(\"piplus\",\"${target}\",${mom}\)

if [ "x" == "x$G4REGRESSION" ]; then
   echo "Regression testing is NOT requested"
else
   echo "G4REGRESSION says: $G4REGRESSION"
$ROOTSYS/bin/root -b -p -q HARPRegre.C\(\"piplus\",\"${target}\",\"${mom}\"\)
fi
...
#
. batch/copy_results.sh ${G4RELEASE} piplus
#
if [ "x" == "x$G4REGRESSION" ]; then
