...
extern void SQLCompose( int,
                        string, string, string, string, string,
	                string, string,
	                string,
		        string,
	                string, string, // x-y axis label(s)
	                string,
	                string,
	                string,
	                int );
...
int main( int argc, char** argv )
{
   ...
   int nsecNA61 = 3;
   int nmodelsHE = 3;
   string HEmodels[3]  = { "ftfp", "qgsp", "qgsp-g4lund-str-fragm" };
   int nbinsNA61 = 10;
   ...
   {
      for ( int im=0; im<nmodelsHE; ++im )
      {
         string histoname = secondaries[is] + "_" + "dNdxF";
	 SQLCompose( 19, "proton", "C", "", "158.0GeV", secondaries[is],
	         HEmodels[im], 
   ...
   for ( int is=0; is<nsecNA61; ++is )
   {
         ...
         for ( int ith=0; ith<nbinsNA61; ++ith )
	 {
	    ...
	    SQLCompose( 19, "proton", "C", "", "31.0GeV", secondaries[is],
	                HEmodels[im], 
		        g4version,
		        histoname,
			reaction_details,
   ...
   for ( int im=0; im<nmodelsHE; ++im )
   {
      SQLCompose( 19, "proton", "C", "", "31.0GeV", "K+,pi+",
	          HEmodels[im], 
		  g4version,
		  "kplus2piplus_20_140",
   ...
   int nmodelsIE = 3;
   string IEmodels[3]  = { "ftfp", "bertini", "inclxx" }; // no MC results for inclxx for until 10.1.p02
	    ...
	    for ( int imm=0; imm<nmomHARP; ++imm )
	    {
	       for ( int is=0; is<nsecHARP; ++is )
	          ...
	          for ( int ia=0; ia<4; ++ia )
	          {
	             ...
	             SQLCompose( 19, beamHARP[ib], targetHARP[it], "", momHARP[imm], secondaries[is],
	                         IEmodels[im], 
		                 g4version,
		                 histoname,
			         reaction_details,
