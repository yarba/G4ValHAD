   ...
   : TstHistoSet(htitle)
{
  ...
  // 7 theta bins - 0.-20, 20-40, 40-60, 60-100, 100-140, 140-180, 180-240
  //
  
  fNSecProtonThetaBins = new int[7];
  fSecProtonBins = new double*[7];
  
  // NOTE: There's a "gap" (no data) at p=1.-1.6 in ALL theta bins.
  //       But it needs to be "padded" in the binning, for the uniformity.
  
  fNSecProtonThetaBins[0] = 15; // 14+1
  fNSecProtonThetaBins[1] = 39; // 38+1
  fNSecProtonThetaBins[2] = 39; // 38+1
  fNSecProtonThetaBins[3] = 30; // 29+1
  fNSecProtonThetaBins[4] = 22; // 21+1
  fNSecProtonThetaBins[5] = 18; // 17+1
  fNSecProtonThetaBins[6] = 14; // 13+1
  
  for ( int ii=0; ii<7; ++ii )
  {
     fSecProtonBins[ii] = new double[fNSecProtonThetaBins[ii]+1];
     int limit = std::min( 9, fNSecProtonThetaBins[ii]+1 );
  // p-binning is 0.1 from 0.2 through 1.0;
     for ( int jj=0; jj<limit; ++jj )
     {
        fSecProtonBins[ii][jj] = 0.1*jj + 0.2;
     }
     if ( fNSecProtonThetaBins[ii] > 9 ) fSecProtonBins[ii][9] = 1.6;
  // from 1.6 onwqards, p-binning is 0.8
     limit = std::max( 0, fNSecProtonThetaBins[ii]-10+1 );
     for ( int jj=0; jj<limit; ++jj )
     {
        fSecProtonBins[ii][jj+10] = 0.8*jj + 2.4; // 2.4 = 1.6+0.8
     }
  }
  
/* cross-check printouts
  std::cout << " PROTON BUSINESS " << std::endl;
  ...
  for ( int ii=0; ii<7; ++ii )
  {
     ...
     for ( int jj=0; jj<fNSecProtonThetaBins[ii]+1; ++jj )
     {
        std::cout << fSecProtonBins[ii][jj] << " " ;
     }
     std::cout << std::endl;
  }
*/
...
//  fHistoSecProton.push_back( new TH1F("protonMult_0_20",title.c_str(),250,0.,25.) ); 
  fHistoSecProton.push_back( new TH1F("protonMult_0_20",title.c_str(),fNSecProtonThetaBins[0],fSecProtonBins[0]) ); 
  title = htitle + outcome + " (20<theta<40 (mrad)) ";
//  fHistoSecProton.push_back( new TH1F("protonMult_20_40",title.c_str(),250,0.,25.) ); 
  fHistoSecProton.push_back( new TH1F("protonMult_20_40",title.c_str(),fNSecProtonThetaBins[1],fSecProtonBins[1]) ); 
  title = htitle + outcome + " (40<theta<60 (mrad)) ";
//  fHistoSecProton.push_back( new TH1F("protonMult_40_60",title.c_str(),250,0.,25.) ); 
  fHistoSecProton.push_back( new TH1F("protonMult_40_60",title.c_str(),fNSecProtonThetaBins[2],fSecProtonBins[2]) );
  title = htitle + outcome + " (60<theta<100 (mrad)) ";
//  fHistoSecProton.push_back( new TH1F("protonMult_60_100",title.c_str(),250,0.,25.) ); 
  fHistoSecProton.push_back( new TH1F("protonMult_60_100",title.c_str(),fNSecProtonThetaBins[3],fSecProtonBins[3]) );
  title = htitle + outcome + " (100<theta<140 (mrad)) ";
//  fHistoSecProton.push_back( new TH1F("protonMult_100_140",title.c_str(),150,0.,15.) ); 
  fHistoSecProton.push_back( new TH1F("protonMult_100_140",title.c_str(),fNSecProtonThetaBins[4],fSecProtonBins[4]) );
  title = htitle + outcome + " (140<theta<180 (mrad)) ";
//  fHistoSecProton.push_back( new TH1F("protonMult_140_180",title.c_str(),150,0.,15.) ); 
  fHistoSecProton.push_back( new TH1F("protonMult_140_180",title.c_str(),fNSecProtonThetaBins[5],fSecProtonBins[5]) ); 
  title = htitle + outcome + " (180<theta<240 (mrad)) ";
//  fHistoSecProton.push_back( new TH1F("protonMult_180_240",title.c_str(),150,0.,15.) ); 
  fHistoSecProton.push_back( new TH1F("protonMult_180_240",title.c_str(),fNSecProtonThetaBins[6],fSecProtonBins[6]) );
//
...
//      double scale = 1. / ((double)stat) ;
   ...
   scale = xsec / ((double)stat);

   for ( size_t i=0; i<fHistoProtonP2015.size(); ++i )
   {
      fHistoProtonP2015[i]->Scale(scale,"width");
      fHistoProtonP2015[i]->Write();
   }
   for ( size_t i=0; i<fHistoPiPlusP2015.size(); ++i )
   {
      fHistoPiPlusP2015[i]->Scale(scale,"width");
      fHistoPiPlusP2015[i]->Write();
   }
   for ( size_t i=0; i<fHistoPiMinusP2015.size(); ++i )
   {
      fHistoPiMinusP2015[i]->Scale(scale,"width");
      fHistoPiMinusP2015[i]->Write();
   }
   for ( size_t i=0; i<fHistoKPlusP2015.size(); ++i )
   {
      fHistoKPlusP2015[i]->Scale(scale,"width");
      fHistoKPlusP2015[i]->Write();
   }
   for ( size_t i=0; i<fHistoKMinusP2015.size(); ++i )
   {
      fHistoKMinusP2015[i]->Scale(scale,"width");
      fHistoKMinusP2015[i]->Write();
   }
   for ( size_t i=0; i<fHistoK0sP2015.size(); ++i )
   {
      fHistoK0sP2015[i]->Scale(scale,"width");
      fHistoK0sP2015[i]->Write();
   }
   for ( size_t i=0; i<fHistoLambdaP2015.size(); ++i )
   {
      fHistoLambdaP2015[i]->Scale(scale,"width");
